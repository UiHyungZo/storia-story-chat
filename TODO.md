...
- [ ] **(2026-09-10/11) iOS 릴리스 파이프라인 — `build_app`에서 계속 막힘. 원인 확정, 해결책은 미정.** 상세는 `HANDOFF.md` 맨 위 항목 + `docs/cicd.md` 8절 디버깅 로그. 요약:
  ...
  - GitHub Secrets 13/14 입력됨 (Firebase 2 + iOS 7 + Android keystore 4). `PLAY_SERVICE_ACCOUNT_JSON_B64`만 미입력 — 아래 항목대로 이제 착수 가능.
  - `app.json` `aps-environment` → `production` 완료. App ID + provisioning profile "Storia App Store" 발급 완료. ASC 앱 레코드 생성 여부 미확인.
- [x] **(2026-09-10) Google Play 개발자 계정 본인 인증 완료.** Play 앱 생성/서비스계정 JSON 발급/`PLAY_SERVICE_ACCOUNT_JSON_B64` 시크릿 입력/최초 AAB 수동 업로드로 이제 진행 가능(사람이 할 일, 아직 미착수).
