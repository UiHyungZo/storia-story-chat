...
- [ ] **(2026-09-03) iOS 릴리스 파이프라인 실행 중 — `build_app`에서 막힘.** 상세는 `HANDOFF.md` 맨 위 2026-09-03 항목. 요약:
  - `develop`→`main` ff 완료(둘 다 `be79e79`). `release.yml` 3건 수정: Bundler 2.5.23 핀(`6c8b258`), `npx pod-install` 분리(`db826d1`), `macos-15`+Xcode 선택(`be79e79`). bundler·pod install 통과, **`build_app`(xcodebuild archive) 실패** — 실제 에러가 fastlane 요약에서 잘림. **다음 세션: gym 로그(`/Users/runner/Library/Logs/gym/Storia-Storia.log`) 확보 → 수정.**
  - GitHub Secrets 13/14 입력됨 (Firebase 2 + iOS 7 + Android keystore 4). `PLAY_SERVICE_ACCOUNT_JSON_B64`만 미입력(Google 계정 인증 대기).
  - `app.json` `aps-environment` → `production` 완료(`9feb375`). App ID + provisioning profile "Storia App Store" 발급 완료. ASC 앱 레코드 생성 여부 미확인.
- [ ] **(2026-09-03) Google Play 개발자 계정 본인 인증 대기** — 공문서 업로드(며칠) + 전화번호 + Android 기기 액세스. 완료 전 Play 앱 생성/서비스계정/android job 불가. 3개 다 착수할 것.
