...
- [ ] **시간이 남으면 — 완전한 양방향 실시간(원래 정의의 A안) 확장**: 지금 축소판은 응답을 오디오 URL로 반환하고 클라이언트가 별도로 재생하는데, 이걸 서버가 합성한 TTS를 그 LiveKit 세션에 오디오 트랙으로 다시 publish해서 실시간으로 들려주는 것까지 가는 확장. 리서치 스파이크에서 확인한 핵심 리스크: 이 "서버가 라이브 세션에 오디오 되쏘기" 부분은 LiveKit도 보통 Python/Node **Agent SDK**로 처리하고, 순수 JVM(Spring Boot)에서 하는 표준 경로가 없음.
  - **(2026-08-26) `pythonSideCar` 브랜치를 `develop`에 merge함 — 스켈레톤(`apps/python-sidecar/`, 여전히 미검증) 코드만 반영, 실행 검증은 아직** — LiveKit Agents SDK로 room에 봇처럼 들어가 STT/TTS는 플러그인이 처리하고, 응답 생성은 새로 안 짜고 기존 Spring REST(`/api/conversations/{characterId}/messages`)를 그대로 호출하도록 설계함. 이에 맞춰 `VoiceCallService.createToken()`의 클라이언트 토큰도 `CanSubscribe(true)`로 변경(에이전트가 되쏘는 오디오를 클라이언트가 구독할 수 있어야 하므로). 자격증명이 없어 `pip install`도 실제 room 연결도 전혀 확인 못 함 — 상세 미검증 목록은 `apps/python-sidecar/README.md` 참고. 다음 세션에서 LiveKit 자격증명 확보되면 실제로 `pip install`/room 연결부터 검증할 것.
  - 시도한다면:
...
- [x] **(2026-08-26)** 개인정보처리방침 초안 작성 완료 — [`docs/legal/privacy-policy.md`](./docs/legal/privacy-policy.md). 수집 항목(디바이스ID/대화 내용/음성통화 오디오/FCM 토큰/Sentry 진단정보), 제3자 위탁(Gemini/Google Cloud STT·TTS/LiveKit/FCM/Sentry), 보관·삭제 절차, 아동 정책, 문의처 이메일(`lukaend@naver.com`)까지 포함. **남은 것**: 스토어 제출엔 실제 공개 URL이 필요 — 리포 마크다운 자체는 URL이 아니므로 GitHub Pages/Artifact 등으로 퍼블리시 필요.
- [ ] 스토어 필수 관문 항목 준비: 위 개인정보처리방침 URL 확정(공개 URL로 퍼블리시), Google Play Data Safety 설문, Apple App Privacy 설문 + Export Compliance
...
- [x] **(2026-08-26)** Release 빌드 파이프라인 — Fastlane 대신 **EAS Build** 채택, `apps/client/eas.json` 신규 작성(`development`/`preview`/`production` 빌드 프로파일 + `submit.production` 스켈레톤, `expo-dev-client`가 이미 설치돼 있어 `developmentClient: true` 프로파일 바로 사용 가능). **남은 것**: (1) `eas init`으로 Expo 프로젝트 연결(Expo 계정 로그인 필요 — 계정 생성/로그인은 코드로 대신할 수 없음, `app.json`에 `extra.eas.projectId`가 채워짐), (2) `eas.json`의 `submit.production.ios`(appleId/ascAppId/appleTeamId)와 `android.serviceAccountKeyPath`는 아직 빈 플레이스홀더 — Apple Developer/Google Play Console 값 채워야 함.
