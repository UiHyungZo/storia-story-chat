...
- [x] **(2026-09-02)** **릴리스 파이프라인을 EAS → Fastlane + GitHub Actions로 전환** ([[ADR-008]], `docs/deployment.md`). `apps/client/fastlane/`(`Fastfile` ios/android `beta` lane, `Appfile`, `Matchfile`), `Gemfile`(`fastlane ~> 2.223`), `plugins/withAndroidReleaseSigning.js`(prebuild가 재생성하는 `android/app/build.gradle`에 release signingConfig + `versionCode` override 주입 — 프로퍼티 없으면 debug 폴백, gradle 평가 성공 확인), `.github/workflows/release.yml`(태그 `v*` 또는 수동, iOS=macos-14 `match`+`gym`+`pilot` / Android=ubuntu `bundleRelease`+`supply` internal draft, 빌드번호=`github.run_number`), `eas.json` 삭제. **남은 것 (1회성, 사람이):** ASC API 키 발급 + ASC 앱 레코드 생성, `storia-certificates` private repo + `fastlane match appstore` + deploy key, Android 업로드 keystore 생성, Play Console 앱 생성 + 서비스계정 JSON + 최초 AAB 수동 업로드, GitHub Secrets 15개 입력(표는 `docs/deployment.md`), `app.json` `aps-environment` → `production`.
