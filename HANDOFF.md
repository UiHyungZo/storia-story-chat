...
- **(2026-09-02) 릴리스 파이프라인 = Fastlane + GitHub Actions** (EAS 폐기, `eas.json` 삭제 — [[ADR-008]]). `apps/client/fastlane/` + `.github/workflows/release.yml` + `plugins/withAndroidReleaseSigning.js`. 파이프라인 코드는 완성·gradle 평가까지 검증. **iOS 서명은 `match` 안 쓰고 `.p12`+profile 을 임시 keychain 에 import — Aran(`~/Desktop/Aran/Aran/.github/workflows/cd.yml`)과 동일.** Distribution 인증서·ASC API 키는 팀 단위라 Aran repo Secrets 재사용, `com.storia.client` provisioning profile 만 새로. 실행 전 1회성 셋업 + GitHub Secrets 필요 — 전체 런북은 `docs/deployment.md`.
