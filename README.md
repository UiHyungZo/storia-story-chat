...
Week 5 코드 작성 완료(실행 검증 전): RN 클라이언트(캐릭터 목록/채팅방 UI, REST + WebSocket 스트리밍 연동, WebSocket 재연결, 로딩/오류/재시도 UI, AsyncStorage 로컬 캐시, Swift Native Module로 Haptic+포그라운드 로컬 알림, 턴제 음성 통화 — STT/TTS 재생) + Spring Boot 백엔드(REST API, STOMP WebSocket, Gemini 스트리밍 연동, DB 스키마, 캐릭터 시딩, CORS, FCM 푸시 백엔드 스켈레톤, TTS 합성 엔드포인트). 상세 현황은 [HANDOFF](./HANDOFF.md) 참고.
